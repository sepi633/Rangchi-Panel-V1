<template>
    ...
    <UCard>
        ...
        <FormDiscussionForum  :is-new="isNew"  :form-data="selected" v-model="openFormModal" @reload="loadData"/>
        <FormDiscussionForumChild   :discussionForumId="selected.id" v-model="openChildModal" @reload="loadData"/>
        <FormDelete :route="'/api/DiscussionForum/DiscussionForum'" formTitle="تالار" @reload="loadData" :selectedId="selected.id" v-model="openDeleteModal"/> 
        <template #footer>
...
const Actions = (row) => [
  ...
  [{
    ...
    click:()=>navigateTo('/DiscussionTopic/'+row.id)

  }],
  [{
    label: 'عناوین',
    icon: 'i-heroicons-bars-4',
    click: () => {openChildModal.value=true; selected.value = { ...row } }
...
async function loadData(){
    try {
      ...
      delete config.list
      console.log(config);
      let res =await $fetch('/api/DiscussionForum/DiscussionForums',{
          method:'GET',
          query:config
        })
        filter.value=res
