<template>
    <div class="p-6 sm:p-10 space-y-6">
        ...
        <div class="lg:w-1/4 w-1/2 p-1 " v-for="item in deliveries?.list" :key="item" :title="item">
            <div class="bg-white rounded p-2 border flex flex-col justify-center items-center min-h-[100px] item-box">
               ...
               <div class="flex  w-full overflow-hidden ">
                  (<template v-for="feature in item?.featureItems">{{ feature?.name }} -</template>)
               </div>
                ...
                <div class="bg-[#1f2937] w-full mt-3 rounded flex p-1 justify-around">
                        ...
                        <!-- <button @click="deleteItem(item.id)"> -->
                            <button class="flex text-white " @click="openDeleteModal(item?.id)">
                        ...
                        <NuxtLink :to="`/featuresitem?id=${item?.id}&isGroup=${false}`" class="flex text-white " >
        ...
        <ModalDelete v-if="showDeleteModal" @closeModal="()=>{showDeleteModal=false}" @haneler="deleteItem" />

        <Paging class="w-full" :pageCount="parseInt(deliveries?.totalCount / deliveries?.pageSize) + 1" :pageIndex="deliveries?.pageIndex"
