...
مدیریت پکیج های امتیاز</p>
            ...
            square
            variant="solid"
            to="/ScoreToRebatePackage/report"
          /> -->
              <UButton
                ...
                square
                variant="solid"
                to="/ScoreToRebatePackage/insertnew"
                />
                <!-- @click="newForum" -->
            </div>
          </div>
      ...
      <UTable   :rows="filter?.list" :columns="columns">            
          ...
          <template #rebate-data="{ row }">
            <div>
              {{ row.isPriceRebate ? row.priceValue + ' تومان' :row.priceValue+ ' %'}}
            </div>
          </template>
      ...
      <FormDelete :route="'/api/ScoreToRebatePackage/ScoreToRebatePackage'" formTitle="اشتراک" @reload="loadData" :selectedId="selected.id" v-model="openDeleteModal"/> 
      <template #footer>
...
[{
  ...
  click:() =>navigateTo('/ScoreToRebatePackage/insert'+row.id)
}],
[{
    label: 'حذف',
...
[{
  ...
  click:() =>navigateTo('/discount/insertnew?typeId=42&ScoreToRebatePackageId='+row?.id)
}],
[{
  label: 'لیست تخفیف',
  icon: 'i-heroicons-percent-badge',
  click:() =>navigateTo('/discount?ScoreToRebatePackageId='+row?.id+'&isAvailable='+true)
}],
[{
  label: 'دسته بندی های مرتبط',
  icon: 'i-heroicons-circle-stack',
  click:() =>navigateTo('/categoryScoreToRebatePackage?id='+row?.id+'&name='+row?.name)
}],
...
async function loadData(){
  try {
    ...
    delete config.list
    console.log(config);
    let res =await $fetch('/api/ScoreToRebatePackage/ScoreToRebatePackages',{
        method:'GET',
        query:config
      })
      filter.value=res
