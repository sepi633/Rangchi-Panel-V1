            ...
            <div class="bg-white rounded p-2 border flex flex-col justify-center items-center min-h-[100px] item-box">
                ...
                <div class="bg-[#1f2937] w-full mt-3 rounded flex p-1 justify-around">
                        <!-- <button @click="getItem(item.id)"> -->
                            <NuxtLink class="flex text-white" :to="`/featuresitem/insert${item?.id}?id=${route.query.id}`">
...
const pageLoader = async () => {
    ...
    try {
        ...
        const data = await $fetch('/api/featureItem/featureItems', {
            method: 'GET',
            headers,
            query: {
                id:route.query.id
...
const deleteItem = async () => {
  try {
    const res = await $fetch('/api/featureItem/featureItem', {
      method: 'DELETE',
      query:{
        id: elId.value
      }
