               ...
               <div v-if="isAlert" class=" bg-red-400 border text-white rounded text-center p-1 mt-2 border-red-600">
                ...
                <div class="acardeon content-box">
                    ...
                    <div class="body-box flex flex-wrap p-2 items-start hidden">
                        ...
                        <div class="input-box flex flex-col p-1">
                            <label for=""> نوع امتیاز: </label>
                            <select class="bg-white border p-1" v-model="data.scoreTypeId">
...
loading.value=true
    if(data.value.name == '' || data.value.amount < 0 || isIncrease.value ==null){
        ...
        isAlert.value =false;
         try {
        const res = await $fetch(('/api/score/score'), {
            method: "POST",
            body: data.value,
        ...
        if(res.isSuccess){
            console.log(res);
            navigateTo("/user/score-"+route.params.userId)
...
async function gettypes(){
    try {
        const response = await $fetch('/api/scoreType/scoreTypes')
        types.value=response.list||[]
