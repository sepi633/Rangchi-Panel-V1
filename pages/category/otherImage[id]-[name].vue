...
<h1>
  سایر عکس های {{route.params.name}}
</h1>
        ...
        <div class="bg-white p-1 rounded border mt-2 flex flex-wrap">
            <div  class="  w-1/2 lg:w-1/6 p-1 relative" v-for="item in imageList"> 
                <!-- {{ item }} -->
                <button class="absolute bg-red-300 top-0 left-0 p-1 rounded text-red-700" @click="()=>{showDeleteModal=true; elId=item?.id}">
                    حذف
                </button>
            <img class="w-full h-full border rounded p-1"  :src="showImageBaseUrl+item?.picture?.url" />
...
const getPictures = async () => {
  try {
    ...
    const data = await $fetch("/api/categoryPicture/categoryPicture", {
      method: "GET",
      headers,
      query: {
...
const sendImageId = (imageId)=>{
    ...
    id.value = imageId;
    newPost.value.categoryId = route?.params?.id;
...
const submitInfo = async () => {
    ...
    try {
      const headers = useRequestHeaders(["cookie"]);
      const insert = await $fetch("/api/categoryPicture/categoryPicture", {
        method: "POST",
        body: newPost.value,
        headers: {
