<template>
    <div class="p-6 sm:p-10 space-y-6">
        <div class="flex flex-wrap">
            ...
            <NuxtLink class="bg-white rounded p-2 border flex justify-center items-center min-h-[100px] cursor-pointer" :to="`/varietyItem/insertnew?id=${route.query.id}`">
            ...
            <div class="bg-white rounded p-2 border flex flex-col justify-center items-center min-h-[100px] item-box">
                ...
                <div class="bg-[#1f2937] w-full mt-3 rounded flex p-1 justify-around">
                        <!-- <button @click="getItem(item.id)"> -->
                            <NuxtLink class="flex text-white" :to="`/varietyItem/insert${item?.id}?id=${route.query.id}`">
...
const pageLoader = async () => {
    ...
    try {
        ...
        const data = await $fetch('/api/varietyItem/varietyItems', {
            method: 'GET',
            headers,
            query: {
                id:route.query.id
...
const deleteItem = async (itemId) => {
  try {
    ...
    const data = await $fetch('/api/varietyItem/varietyItem', {
      method: 'DELETE',
      headers,
      query:{
        id: itemId
