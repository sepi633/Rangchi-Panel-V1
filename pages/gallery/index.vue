<template>
  <div class=" p-6 sm:p-10 space-y-6">
    <UCard>
        ...
        <div>
          ...
          <div class="flex justify-between flex-wrap p-2">
            <div  class="w-96 min-w-max shrink-0">
              <CategorySelect v-model="filter.categoryId" :parentId="gallerycategoryId" :isMulti="false" />
            </div>

            <div class="flex justify-end">
              <UButton icon="i-heroicons-plus-small" size="sm" color="primary" square variant="solid"
                to="/gallery/insertnew" />
              <!-- @click="newForum" -->
            </div>
          </div>
        </div>
      ...
      <FormDelete :route="'/api/gallery/gallery'" formTitle="گالری" @reload="loadData" :selectedId="selected.id"
...
const Actions = (row) => [
  [{
    ...
    click: () => navigateTo('/gallery/insert' + row.id)
  }],
  [{
    label: 'حذف',
  ...
  click:() =>navigateTo('/gallery/enLanguage'+row?.id)
}],
  [{
  label: 'ایتم های گالری',
  icon: 'i-heroicons-list-bullet-16-solid',
  click:() =>navigateTo('/gallery/item-'+row?.id)
}],
]
// function select (row) {
//     selected.value=(row)
...
async function loadData() {
  ...
  console.log(config);
    let res = await $fetch('/api/gallery/gallerys', {
      method: 'GET',
      query: config
    })
    filter.value = res
