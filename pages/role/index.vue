...
نقش ها           </p>
        ...
        <FormDelete :route="'/api/role/role'" formTitle="نقش" @reload="loadData" :selectedId="selected.id" v-model="openDeleteModal"/> 
        <template #footer>
  ...
  [{
    ...
    click:() =>navigateTo('/role/insert'+row.id)
  }],
  [{
    label: 'حذف',
  ...
  async function loadData(){
    try {
      ...
      delete config.list
      console.log(config);
      let res =await $fetch('/api/role/role',{
          method:'GET',
          query:config
        })
        filter.value=res
