                ...
                <div class="acardeon content-box">
                    ...
                    <div class="body-box flex flex-wrap p-2 items-start hidden">
                        <div class="w-1/2">
                            ...
                            <input class="border w-full h-[40px] mt-2" type="text" name="" v-model="newRole.name" id="">
                        </div>
                        <div class="input-box flex flex-col p-1">
                            <label for=""> {{ $t('label') }}:</label>
                            <input class="border w-full h-[40px] mt-2" type="text" name="" v-model="newRole.label" id="">
                        </div>
...
try {
    ...
    const data = await $fetch('/api/role/roleItem', {
        method: 'GET',
        headers,
        query: {
            userId: id
...
loading.value=true
    if(type=='insert'){
        try {
        // const headers = useRequestHeaders(['cookie'])
        const insert = await $fetch(('/api/role/role'), {
            method: "POST",
            body: newRole.value,
            headers: {
