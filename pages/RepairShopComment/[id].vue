<template>
       <div class="p-6 sm:p-10 ">
            ...
            <NuxtLink to="/RepairShopComment" class="btn bg-rose-700 px-2 py-1.5 text-[#fff] ms-3 ">{{ $t('cancel') }}</NuxtLink>
       </div>
...
//   loader.value=true
  try {
    ...
    const respons = await $fetch('/api/RepairShopComment/comment', {
      method: 'GET',
      query: {
                id: route.params.id
            }
...
const editeComment = async () =>{
    ...
    const data = await $fetch('/api/RepairShopComment/edit', {
      method: 'PUT',
      body:{
