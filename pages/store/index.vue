...
مدیریت فروشگاه ها</p>
      ...
      <FormDelete :route="'/api/store/store'" formTitle="تنوع محصول" @reload="loadData" :selectedId="selected.id" v-model="openDeleteModal"/> 
      <template #footer>
...
[{
  ...
  click:() =>navigateTo('/store/insert'+row.id)
}],
[{
    label: 'حذف',
...
async function loadData(){
  try {
    ...
    delete config.list
    console.log(config);
    let res =await $fetch('/api/store/store',{
        method:'GET',
        query:config
      })
      filter.value=res
