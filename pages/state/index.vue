<template>
  <div class=" p-6 sm:p-10 space-y-6">
    <UCard>
        ...
        <div>
          ...
          <div class="flex justify-between flex-wrap p-2">
            <div  class="w-96 min-w-max shrink-0">
              <!-- <CategorySelect v-model="filter.categoryId" :parentId="statecategoryId" :isMulti="false" /> -->
            </div>
      ...
      <FormDelete :route="'/api/state/state'" formTitle="استان" @reload="loadData" :selectedId="selected.id"
...
const {sortBy,pageFrom,pageTo,loadData}=useTable('/api/state/states',sort,filter)
...
const Actions = (row) => [
  [{
    ...
    click: () => navigateTo('/state/insert' + row.id)
  }],
  [{
    label: 'حذف',
