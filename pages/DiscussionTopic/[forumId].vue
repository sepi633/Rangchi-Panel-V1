<template>
  <div class="p-6 sm:p-10 space-y-6">
    <UCard>
      ...
      >
        ...
        <template #createDate-data="{ row }">
          <span>
            {{
              row.createDate
                ? moment(row.createDate).locale("fa").format("YYYY/MM/DD")
                : ""
            }}
          </span>
        </template>
        ...
        <template #active-data="{ row }">
          <UIcon
            ...
            size="lg"
            :name="
              row.active ? 'i-heroicons-check-16-solid' : 'i-heroicons-x-mark-16-solid'
            "
          />
        </template>
        <template #closed-data="{ row }">
          <UIcon
            :class="[row.closed ? 'text-red-400' : 'text-green-400', 'w-8 h-8']"
            size="lg"
            :name="row.closed ? 'i-heroicons-lock-closed' : 'i-heroicons-lock-open'"
          />
        </template>
      ...
      />
      <FormDelete
        :route="'/api/DiscussionTpoic/DiscussionTpoic'"
        formTitle="تاپیک"
        @reload="loadData"
        :selectedId="selected.id"
        v-model="openDeleteModal"
      />
      <FormDiscussionTopicConfirm
        :form-data="selected"
...
const sortBy = computed(() => {
  if (
    (sort.value.column == "createDate" || sort.value.column == "id") &&
...
const filter = ref({
  ...
  pageIndex: 1,
  pageSize: 15,
  DiscussionForumId: route.params.forumId,
  cityId:null,
  stateId:null
});
...
watch([() => filter.value.pageIndex, () => sortBy.value,()=>filter.value.stateId,()=>filter.value.cityId], async () => {
  await loadData();
});
