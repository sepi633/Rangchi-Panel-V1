<template>
    <div class="p-6 sm:p-10 space-y-6">
        ...
        <input
          class="border mx-2"
          type="checkbox"
          @change="newData.active = !newData.active"
...
try {
    ...
    const data = await $fetch('/api/DiscountGroup/DiscountGroupItem', {
        method: 'GET',
        headers,
        query: {
            varietyId: id
...
const submitInfo = async (type) => {
    ...
    if(type=='insert'){
        ...
        const insert = await $fetch('/api/DiscountGroup/DiscountGroup', {
            method: "POST",
            body: newData.value,
            headers: {
