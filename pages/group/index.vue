<template>
    <div class="p-6 sm:p-10 space-y-6">
        <div class="flex flex-wrap">
            ...
            <NuxtLink class="bg-white rounded p-2 border flex justify-center items-center min-h-[100px] cursor-pointer" to="/group/insertnew">
        ...
        <div class="lg:w-3/12 w-1/2 p-1 " v-for="item in allDetails?.list" :key="item" :title="item">
            <div class="bg-white rounded p-2 border flex flex-col justify-center items-center min-h-[100px] item-box">
               ...
               <ul class="flex childs">
                  (<li class="mx-1" v-for="feature in item?.featureItems">{{ feature?.name }} <span>-</span></li>)
               </ul>
            
                <br>
                <!--{{ item.label }} -->
                <div class="bg-[#1f2937] w-full mt-3 rounded flex p-1 justify-around">
                        <!-- <button @click="getItem(item.id)"> -->
                            <NuxtLink class="flex text-white" :to="`/Feature/insert${item?.id}`">
                        ...
                        <NuxtLink :to="`/featuresitem?id=${item?.id}&isGroup=${true}`" class="flex text-white " >
...
const pageLoader = async () => {
    ...
    try {
        ...
        const data = await $fetch('/api/feature/features', {
            method: 'GET',
            headers,
