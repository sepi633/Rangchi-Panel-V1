...
مدیریت برند ها</p>
      ...
      <FormDelete :route="'/api/brand/brand'" formTitle="برند" @reload="loadData" :selectedId="selected.id" v-model="openDeleteModal"/> 
      <template #footer>
...
[{
  ...
  click:() =>navigateTo('/brand/insert'+row.id)
}],
[{
    label: 'حذف',
...
[{
  ...
  click:() =>navigateTo('/discount/insertnew?typeId=42&brandId='+row?.id)
}],
[{
  label: 'لیست تخفیف',
  icon: 'i-heroicons-percent-badge',
  click:() =>navigateTo('/discount?BrandId='+row?.id+'&isAvailable='+true)
}],
[{
  label: 'دسته بندی های مرتبط',
  icon: 'i-heroicons-circle-stack',
  click:() =>navigateTo('/categoryBrand?id='+row?.id+'&name='+row?.name)
}],
[{
  label: 'زبان انگلیسی',
  icon: 'i-heroicons-language-16-solid',
  click:() =>navigateTo('/brand/enLanguage'+row?.id)
}],
...
async function loadData(){
  try {
    ...
    delete config.list
    console.log(config);
    let res =await $fetch('/api/brand/brands',{
        method:'GET',
        query:config
      })
      filter.value=res
