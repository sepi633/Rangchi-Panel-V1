<template>
  <div class="flex flex-wrap">
    <h1 class="w-full text-xl font-bold">زبان عربی</h1>
    ...
    <div class="inputgroup w-full lg:w-1/2">
      ...
      <input
        type="text"
        v-model="categoryDataLang.seoFieldLangDto.seoPictureAlt"
      />
    </div>
    <div class="inputgroup w-full lg:w-1/2">
      <label for=""> خلاصه</label>
      <textarea
        v-model="categoryDataLang.seoFieldLangDto.summary"
        name=""
        id=""
        cols="30"
        rows="10"
    ...
    <div class="inputgroup w-full lg:w-1/2">
      ...
      <textarea
        v-model="categoryDataLang.seoFieldLangDto.seoMinDescription"
        name=""
        id=""
        cols="30"
        rows="10"
    ...
    <div class="inputgroup">
      ...
      <RichEditor v-model="categoryDataLang.seoFieldLangDto.seoDescription" />
    </div>
...
const getCategoryItem = async (id) => {
  try {
    ...
    const data = await $fetch("/api/product/productLanguage", {
      method: "GET",
      headers,
      query: {
        catId: id,
    ...
    if (data?.list) {
      console.log(data?.list.find(
        (item) => item.languageId == 3
      ));
      
      categoryDataLang.value.seoFieldLangDto =( data?.list.find((item) => item.languageId == 3 ) ||  {
          languageId: 3,
          seoH1: "",
          seoMinDescription: "",
          seoDescription: "",
...
const submitInfo = async () => {
  ...
  try {
    const headers = useRequestHeaders(["cookie"]);
    const insert = await $fetch("/api/product/productLanguage", {
      method: "POST",
