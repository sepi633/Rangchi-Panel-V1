      ...
      <UCard>
        <template #header>
          <div class="flex gap-4">
            <UFormGroup class="flex-1" label="سکشن " name="name">
              <USelect
              v-model="categoryDetailId"
              :options="categoryDetails?.list"
...
const fromDate = ref();
const toDate = ref();
const categoryDetailId = ref();
const categoryId = ref();
const unreadCount = ref(0);
const successCount = ref(0);
const rejectCount = ref(0);
watch([() => fromDate.value, () => toDate.value, () => categoryId.value,()=>categoryDetailId], async () => {
  await loadData1();
  await loadData2();
  await loadData3();
});
...
async function loadData1() {
  try {
    let res = await $fetch("/api/categoryDetailComment/comments", {
      method: "GET",
      query: {
