<template>
       <div class="p-6 sm:p-10 ">
            ...
            <NuxtLink to="/categoryDetailComment" class="btn bg-rose-700 px-2 py-1.5 text-[#fff] ms-3 ">{{ $t('cancel') }}</NuxtLink>
       </div>
...
//   loader.value=true
  try {
    ...
    const respons = await $fetch('/api/categoryDetailComment/comment', {
      method: 'GET',
      query: {
                id: route.params.id
            }
...
const editeComment = async () =>{
    ...
    const data = await $fetch('/api/categoryDetailComment/edit', {
      method: 'PUT',
      body:{
        ...
        "categoryId": comment.value.categoryId,
  ...
  ;
  if(data?.isSuccess){
    navigateTo("/categoryDetailComment")
