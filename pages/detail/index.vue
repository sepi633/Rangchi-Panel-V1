<template>
  <section>
    <div class="title relative flex justify-between items-center py-2">
      ...
      <nuxtLink to="/detail/insertnew"
        class="btn border bg-white p-1 rounded flex items-center justify-center px-3 border-[#1f2937] text-[#1f2937]">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd"
            ...
            fill="black" style="fill: #1f2937"></path>
        </svg>

        <span>{{ $t('addNewItem') }}</span>
      </nuxtLink>
      <nuxtLink to="/detail/base"
        class="btn  border bg-white p-1 rounded flex items-center justify-center  px-3 border-[#1f2937] text-[#1f2937]">


        <span>{{ $t('BaseDetail') }}</span>
      </nuxtLink>
    </div>
    <div class="table-function flex flex-wrap items-end mb-3">


      <!-- <div class="lg:w-2/12 w-full flex flex-wrap p-1">
        <span class="block"> {{ $t("CategoryName") }}: </span>
        <select
          class="form-select bg-white rounded w-full p-1 border"
          @change="
            (e) => {
              defaultData.CategoryId = e.target.value;
              ;
            }
          "
        >
          <option value="">{{ $t("PleaseSelect") }}</option>
          <option v-for="item in categoris?.children" :value="item?.id" :key="item?.id">
            {{ item?.name }}
          </option>
        </select>
      </div> -->
      <div class="lg:w-3/12 w-full p-1">
        <span for="exampleInputPassword1" class="form-label">{{ $t("title") }}:</span>
        <input type="text" class="form-control border rounded w-full p-1" v-model="filter.CategoryLabel"
          id="name" />
      </div>
      <div class="lg:w-1/12 w-full p-1">
        <button class="btn bg-[#136b57] text-white p-2 border border-[#136b57] rounded w-full flex items-center"
          @click="pageLoader">
          ...
          <!-- <span class="mx-1">اعمال فیلترینگ </span> -->
        </button>
      </div>
    </div>
    <div class="item-parent " v-for="item in filter?.list" :key="item">
      <div class="p-3 px-4 bg-white my-1 rounded border flex flex-wrap hover:bg-[#0097fb] cursor-pointer"
        @click="acardeon">
        <strong class="min-w-[100px] block">{{ item?.title }}:</strong>
        <span dir="ltr" class="mx-2">{{ item?.value }}</span>
      </div>
      <div class="item-content p-3 px-4 bg-white my-1 mb-2 rounded border shadow hidden">
        <ul>
          <li>
            <span>{{ $t("Category") }}:</span><span>{{ item?.type?.name }}</span>
          </li>
          <li>
            <span>{{ $t("label") }}:</span><span>{{ item?.label }}</span>
          </li>
          <li>
            <span>{{ $t("id") }}:</span><span>{{ item?.id }}</span>
          </li>
          <li>
        ...
        <div class="flex justify-end">
          <button class="btn bg-[#713632] lg:w-2/12 w-1/2 my-3 px-3 py-1 text-white flex items-center"
            ...
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd"
                ...
                fill="black" style="fill: #fff"></path>
            </svg>

            {{ $t("Delete") }}
          </button>
          <button class="btn bg-[#2563eb] lg:w-2/12 w-1/2 my-3 mx-1 px-3 py-1 text-white flex items-center"
...
watch([() => filter.value.pageIndex,()=>filter.value.CategoryLabel], () => {
  pageLoader();
})
...
const pageLoader = async () => {
  ...
  try {
    ...
    const data = await $fetch("/api/details/details", {
      method: "GET",
      headers,
      query: config
    });
...
const acardeon = (event) => {
  // alert("yes")
  const allContent = document.getElementsByClassName("item-content");
  for (var i = 0; i < allContent.length; i++) {
    allContent[i].classList.add("hidden");
  }
  ;
  const parent = event.target.closest(".item-parent");
