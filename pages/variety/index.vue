...
<template>
  ...
  مدیریت  تنوع محصولات</p>
        ...
        <UTable   :rows="filter?.list" :columns="columns">            
            ...
            <template #insearch-data="{ row }">
              <UIcon :class="[row.insearch ?'text-green-400':'text-red-400','w-8 h-8']" size="lg" :name="row.insearch?'i-heroicons-check-16-solid':'i-heroicons-x-mark-16-solid'"/> 
  
            </template>

        
        </UTable>
        <FormDelete :route="'/api/variety/variety'" formTitle="تنوع محصول" @reload="loadData" :selectedId="selected.id" v-model="openDeleteModal"/> 
        <template #footer>
  ...
  [{
    ...
    click:() =>navigateTo('/variety/insert'+row.id)
  }],
  [{
    label: 'حذف',
  ...
  [{
    ...
    click: () => navigateTo(`/varietyitem?id=${row.id}`)
  }],
  ...
  async function loadData(){
    try {
      ...
      delete config.list
      console.log(config);
      let res =await $fetch('/api/variety/variety',{
          method:'GET',
          query:config
        })
        filter.value=res
...
<template>
    <div class="p-6 sm:p-10 space-y-6">
        <div class="flex flex-wrap">
            ...
            <NuxtLink class="bg-white rounded p-2 border flex justify-center items-center min-h-[100px] cursor-pointer" to="/variety/insertnew">
        ...
        <div class="lg:w-3/12 w-1/2 p-1 " v-for="item in allDetails?.list" :key="item" :title="item">
            <div class="bg-white rounded p-2 border flex flex-col justify-center items-center min-h-[100px] item-box">
        

                <strong class="text-[18px]">{{ item?.name }}</strong>
             
            
                <br>
                <div class="bg-[#1f2937] w-full mt-3 rounded flex p-1 justify-around">
                            <NuxtLink class="flex text-white" :to="`/variety/insert${item?.id}`">
                            ...
                            <button class="flex text-white " @click="()=>{showDeleteModal=true; elId=item?.id}">
                        ...
                        <NuxtLink :to="`/varietyitem?id=${item?.id}`" class="flex text-white " >
                            <svg class="me-1" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_1_20510)">
                                <path style="stroke: rgb(255, 255, 255);" d="M8 16H5.43C3.14 16 2 14.86 2 12.57V5.43C2 3.14 3.14 2 5.43 2H10C12.29 2 13.43 3.14 13.43 5.43" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path style="stroke: rgb(255, 255, 255);" d="M18.5698 22H13.9998C11.7098 22 10.5698 20.86 10.5698 18.57V11.43C10.5698 9.14 11.7098 8 13.9998 8H18.5698C20.8598 8 21.9998 9.14 21.9998 11.43V18.57C21.9998 20.86 20.8598 22 18.5698 22Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path style="stroke: rgb(255, 255, 255);" d="M14.8701 15H18.1301" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path style="stroke: rgb(255, 255, 255);" d="M16.5 16.6301V13.3701" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                </g>
                                <defs>
                                <clipPath id="clip0_1_20510">
        ...
        <Paging class="w-full" :pageCount="Math.ceil(allDetails?.totalCount /  allDetails?.pageSize)" :pageIndex="allDetails?.pageIndex"
