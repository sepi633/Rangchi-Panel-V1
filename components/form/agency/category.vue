<template>
    <UModal v-model="isOpen" prevent-close fullscreen>
        <UCard class="shadow-md bg-grayPrimary-50  bg-opacity-60 ">
            ...
            <UCard>                
                <div class="flex items-center  gap-2">
                    <UForm :state="formData" class="px-8 w-1/2 ">
                        <UFormGroup class="w-full " label="خودرو " name="name">
                        <CategorySelect :parentId="carCategoryId" v-model="formData.categoryId" />
                    </UFormGroup>
                </UForm>
                <UFormGroup class="">
        ...
        <UCard class="mt-4">
            ...
            <FormDelete :route="'/api/AgencyCategory/AgencyCategory'" formTitle="خودرو" @reload="loadData" :selectedId="selected.id"
...
async function loadData() {
    try {
        ...
        console.log(config);
        let res = await $fetch('/api/AgencyCategory/AgencyCategorys', {
            method: 'GET',
            query: config
        })
        filter.value = res
...
async function submit() {
    if (loading.value) return;
        ...
        try {
            let res = await $fetch("/api/AgencyCategory/AgencyCategory", {
                method: "POST",
                body: formData.value,
            });
            if (res.isSuccess) {
