<template>
    <ul class="">
       <li  v-for="item in props?.data" :key="item?.id">
        ...
        <label :for="item?.id" class="flex items-center cursor-pointer p-2 hover:bg-blue-100" >
            ...
            <svg v-if="item.children.length > 0" class="mx-2 cursor-pointer" @click="childrenHandler" width="15" height="15" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <TreeMiniTree v-if="item.children.length > 0" v-model="model" class="p-1 px-4" :data="item.children" />
    </li> 
    </ul>
</template>
