<template>
  <ul class="flex flex-wrap max-h-[50vh] overflow-auto">
    <li class="w-1/2" v-for="item in props?.data" :key="item?.id">
      <div class="flex">
        <input
          v-model="catId"
          :value="item?.id"
        ...
        <label
          ...
          class="flex items-center cursor-pointer p-2 hover:bg-blue-100 text-[12px] border w-full"
        >
          <span>{{ item?.name }}</span>
          ...
          <svg
            ...
            class="mx-2 transition-transform duration-200 ease-in-out"
            :class="{ 'rotate-180': isOpenMap[item.id] }"
            width="15"
            height="15"
            viewBox="0 0 24 24"
            fill="none"
      ...
      <TreeBannerMiniTree
        v-if="item.children.length > 0"
        :class="{ hidden: !isOpenMap[item.id] }"
        class="p-1 px-4"
        :data="item.children"
        v-model="catId"
      />
...
const catId = defineModel()
const props = defineProps(['data'])

const isOpenMap = ref({})

const toggleChildren = (id) => {
  isOpenMap.value[id] = !isOpenMap.value[id]
}
</script>
