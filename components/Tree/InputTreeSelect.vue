      ...
      <ul class="tree">
        ...
        <div class="branch-box" v-if="item.parentId==null">
          ...
          <input class="persmission-input" :class="{'ms-5 ':item?.children.length == 0}" :id="item.name" type="checkbox" :value="JSON.stringify(item)"
            ...
            @change="allChildsPermission" :name="item.name" /> 
          <label :for="item.name"> {{ item.name}}</label>
        </div>
        <ul >
          
      <TreeInputTreeBranch @checkChildren='allChildsPermission'  :data="item?.children" :allcheck="true"></TreeInputTreeBranch>
...
const allChildsPermission = async (e) => {
  ...
  ;
  const parent = e.target.closest('.branch-box')
